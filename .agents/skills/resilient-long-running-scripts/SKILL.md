...
when_to_use: "Do NOT use for simple one-off scripts. DO use for anything running >1 minute unattended, iterating over units of work, or that could fail silently."
license: MIT
metadata:
  version: "1.0.0"
  author: johninan
...
const heal = (item, error) => {
  ...
  try {
    const r = spawnSync('codex', ['--print', prompt], { timeout: 120_000, encoding: 'utf8' });
...
| `codex --print` returns prose | Prompt explicitly: `Respond with JSON only: {...}` |
...
| Node.js: string-interpolated shell command | Use `spawnSync('codex', ['--print', prompt])` — avoids quoting bugs on Windows |
